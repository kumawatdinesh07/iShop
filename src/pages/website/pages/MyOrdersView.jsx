...
function MyOrdersView() {
  ...
  let { prodImgUrl } = useSelector(store => store.product);
  const { order } = useSelector(store => store.order);
  const { order_id } = useParams();

  const orderDetails = order.filter(
    (ord) => {
      if (ord._id === order_id) {
        return true;
      } else {
        ...
        <>
          ...
          <div className="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
            ...
            <div className="flex justify-start item-start space-y-2 flex-col">
              <div className=" dark:text-white text-xl md:text-4xl font-semibold leading-7 lg:leading-9 text-gray-800">
                Order #{order_id}
              </div>
            ...
            <div className="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
              <div className="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
                ...
                <div className="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
                  ...
                  {
                    orderDetails[0].order_details.map(
                      (prod, index) => {
                        return (
                                ...
                                <div className="flex justify-between space-x-8 items-start w-full">
                                  <p className="text-base dark:text-white xl:text-lg leading-6">
                                    ₹{prod.final}
                                    {
                                      prod.discount !== 0
                                        ?
                                        <span className="text-red-300 line-through"> ₹{prod.price}</span>
                                        :
                                        ""
                                    }
                                  </p>
                ...
                <div className="flex justify-center flex-col md:flex-row flex-col items-stretch w-full space-y-4 md:space-y-0 md:space-x-6 xl:space-x-8">
                  ...
                  <div className="flex flex-col justify-center px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
                      ...
                      <div className="flex justify-center items-center space-x-4">
                        ...
                        <div className="flex flex-col justify-start items-center">
                          <p className="text-lg leading-6 dark:text-white font-semibold text-gray-800">
                            DPD Delivery
                            <br />
                            <span className="font-normal">Delivery with in Working Day</span>
                          </p>
                        </div>
                      </div>
                    ...
                    <Link to={"/profile/tracking-order"}>
                  ...
                  <div className="flex flex-col justify-start items-start flex-shrink-0">
                    <div className="flex justify-center w-full md:justify-start items-center space-x-4 py-8 border-b border-gray-200">
                      ...
                      <div className="flex justify-start items-start flex-col space-y-2">
                        <p className="text-base dark:text-white font-semibold leading-4 text-left text-gray-800">
                          {orderDetails[0].user_details.name}
                        </p>
                        <p className="text-sm dark:text-gray-300 leading-5 text-gray-600">
                          {order.length - 1} Previous Orders
                        </p>
                      </div>
                    </div>
                    <div className="flex justify-center text-gray-800 dark:text-white md:justify-start items-center space-x-4 py-4 border-b border-gray-200 w-full">
                      ...
                      <p className="cursor-pointer text-sm leading-5 ">
                        {orderDetails[0].user_details.email}
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row items-center md:items-start">
                      <div className="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4 xl:mt-8">
                        ...
                        <p className="w-48 lg:w-full dark:text-gray-300 xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">
                          {orderDetails[0].user_details.address}
                        </p>
                      </div>
